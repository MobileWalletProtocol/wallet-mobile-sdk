...
public enum EncryptedRequestContent: EncryptedContent {
    case handshake(appId: String, callback: URL, initialActions: [Action]?, verificationMethod: VerificationMethod?)
    case request(data: Data)
    
    public func decrypt(with symmetricKey: SymmetricKey?) throws -> RequestContent {
        switch self {
        case let .handshake(appId, callback, initialActions, verificationMethod):
            return .handshake(appId: appId, callback: callback, initialActions: initialActions, verificationMethod: verificationMethod)
